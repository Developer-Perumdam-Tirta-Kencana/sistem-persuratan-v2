        ...
        <div class="max-w-7xl mx-auto">
            ...
            <div class="bg-white shadow-xl border border-slate-100 overflow-hidden">
                ...
                <div class="px-6 sm:px-8 py-6 border-b border-slate-200 bg-gradient-to-r from-slate-50 via-white to-slate-50">
                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        <div>
                            <h3 class="text-xl font-bold text-slate-900">Daftar Surat Payroll</h3>
                    ...
                    <table id="payrollTable" class="w-full text-sm">
                        <thead>
                            <tr class="bg-slate-100 border-b border-slate-200 hover:bg-slate-200 transition-colors">
                                ...
                                <th class="px-6 py-4 text-left font-bold text-slate-700 uppercase tracking-wider text-xs">Tanggal</th>
                                <th class="px-6 py-4 text-left font-bold text-slate-700 uppercase tracking-wider text-xs">Bulan Gaji</th>
                                <th class="px-6 py-4 text-center font-bold text-slate-700 uppercase tracking-wider text-xs">Bank</th>
                                <th class="px-6 py-4 text-right font-bold text-slate-700 uppercase tracking-wider text-xs">Total Nominal</th>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            <tr class="hover:bg-slate-50 transition-colors duration-150">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="font-medium text-slate-900">{{ $letter->nomor_surat }}</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="text-sm text-slate-700">{{ $letter->tanggal_surat->format('d M Y') }}</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="text-sm text-slate-700">{{ $letter->bulan_gaji }}</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <span class="inline-flex items-center px-3 py-1 text-xs font-semibold bg-blue-100 text-blue-800">
                                        {{ $letter->bank_tujuan }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right">
                                    <span class="font-medium text-slate-900">Rp {{ number_format($letter->total_nominal, 0, ',', '.') }}</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="flex justify-center gap-1.5 flex-wrap">
                                        <!-- View Buttons -->
                                        <x-view-dropdown :route="route('payroll-letters.previewFormat', $letter)" :id="$letter->id" />
                                        <a href="{{ route('payroll-letters.edit', $letter) }}" class="inline-flex items-center px-2.5 py-1.5 text-xs font-medium text-indigo-600 hover:text-indigo-900 hover:bg-indigo-50 transition" title="Edit surat">
                            ...
                            <tr>
                                <td colspan="6" class="px-6 py-12">
                                    <div class="text-center">
                                        ...
                                        <p class="text-slate-600 text-sm mb-4">Mulai dengan membuat surat payroll pertama Anda</p>
                                        <a href="{{ route('payroll-letters.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white font-semibold hover:bg-blue-700 transition">
                                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                                            Buat Surat Baru
                                        </a>
                                    </div>
