            ...
            <div class="bg-white shadow-sm sm:rounded-lg border border-gray-200">
                <div class="p-6">
                    <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                        <div>
                            <h1 class="text-2xl font-bold text-gray-900">Pratinjau Surat Payroll</h1>
                            <p class="text-sm text-gray-600">Nomor: {{ $payrollLetter->nomor_surat }}</p>
                            <div class="mt-2 inline-flex items-center px-3 py-1 text-xs font-semibold rounded-full
                                @if($payrollLetter->status === 'disetujui') bg-green-100 text-green-800
                                @elseif($payrollLetter->status === 'ditolak') bg-red-100 text-red-800
                                @else bg-yellow-100 text-yellow-800 @endif">
                                Status: {{ ucfirst(str_replace('_', ' ', $payrollLetter->status ?? 'Menunggu')) }}
                            </div>
                        </div>
                        <div class="flex flex-wrap gap-2">
                            <a href="{{ route('payroll-letters.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-800 font-semibold rounded-lg transition">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                                Kembali ke Daftar
                            </a>
                            <a href="{{ route('payroll-letters.edit', $payrollLetter) }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-lg transition">
                    ...
                    <div class="mt-6 inline-flex rounded-lg border border-gray-200 overflow-hidden">
                        <a href="{{ route('payroll-letters.previewFormat', [$payrollLetter, 'kop' => 1, 'paper' => 'F4']) }}" class="px-4 py-2 text-sm font-medium transition border-r border-gray-200 {{ $withKop ? 'bg-blue-50 text-blue-700' : 'text-gray-600 hover:bg-gray-50' }}">Dengan Kop</a>
                        <a href="{{ route('payroll-letters.previewFormat', [$payrollLetter, 'kop' => 0, 'paper' => 'F4']) }}" class="px-4 py-2 text-sm font-medium transition {{ !$withKop ? 'bg-blue-50 text-blue-700' : 'text-gray-600 hover:bg-gray-50' }}">Tanpa Kop</a>
                    </div>
                    ...
                    <div class="mt-6 flex gap-2 flex-wrap">
                        <a href="{{ route('payroll-letters.exportPdf', [$payrollLetter, 'kop' => $withKop ? '1' : '0']) }}" class="inline-flex items-center px-4 py-2 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-lg transition">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                            Download PDF
                        </a>
                        <a href="{{ route('payroll-letters.exportDocx', [$payrollLetter, 'kop' => $withKop ? '1' : '0']) }}" class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg transition">
                        ...
                        <a href="{{ route('payroll-letters.show', $payrollLetter) }}" class="inline-flex items-center px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-800 font-semibold rounded-lg transition">
