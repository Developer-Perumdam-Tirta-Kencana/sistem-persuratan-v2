<x-app-layout>
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h2 class="font-bold text-2xl md:text-3xl text-slate-900 leading-tight">
                    {{ __('Surat Ketersediaan Air') }}
                </h2>
                <p class="text-sm text-slate-600 mt-2">Kelola dan pantau semua surat penegasan ketersediaan air bersih</p>
            </div>
            <a href="{{ route('water-availability-letters.create') }}" class="inline-flex items-center px-6 py-2.5 bg-gradient-to-r from-cyan-600 to-cyan-700 text-white font-semibold hover:shadow-lg hover:from-cyan-700 hover:to-cyan-800 transition-all duration-200 transform hover:scale-105">
            ...
            <div class="bg-white shadow-xl border border-slate-100 overflow-hidden flex flex-col flex-1">
                ...
                <div class="px-6 py-5 border-b border-slate-200 bg-gradient-to-r from-blue-50 via-white to-blue-50">
                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
                        <div>
                            <h3 class="text-xl font-bold text-slate-900">Daftar Surat Ketersediaan Air</h3>
                            <p class="text-sm text-slate-600 mt-1">{{ $letters->total() }} surat dalam sistem</p>
                        </div>
                        <!-- Search Input -->
                        <div class="w-full sm:w-64 relative">
                            <input type="text" id="waterAvailabilitySearch" placeholder="Cari..." class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
                ...
                <div class="overflow-x-auto px-4 pt-4">
                    <table id="waterAvailabilityTable" class="w-full text-sm">
                        <thead>
                            <tr class="bg-blue-100 border-b border-slate-200">
                                <th class="px-6 py-4 text-left font-bold text-slate-700 uppercase tracking-wider text-xs cursor-pointer hover:bg-blue-200 transition" data-column="status">
                                ...
                                <th class="px-6 py-4 text-left font-bold text-slate-700 uppercase tracking-wider text-xs cursor-pointer hover:bg-blue-200 transition" data-column="pengembang">
                                    <div class="flex items-center gap-2">
                                        Pengembang
                                        <svg class="w-4 h-4 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m0 0h16"/></svg>
                                    </div>
                                </th>
                                <th class="px-6 py-4 text-left font-bold text-slate-700 uppercase tracking-wider text-xs cursor-pointer hover:bg-blue-200 transition" data-column="proyek">
                                    <div class="flex items-center gap-2">
                                        Proyek
                                        <svg class="w-4 h-4 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m0 0h16"/></svg>
                                    </div>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            ...
                            <tr class="hover:bg-blue-50 transition-colors duration-150">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <span class="px-3 py-1 inline-flex text-xs font-semibold bg-green-100 text-green-800">Tersedia</span>
                                    @else
                                    <span class="px-3 py-1 inline-flex text-xs font-semibold bg-red-100 text-red-800">Tidak Tersedia</span>
                                    @endif
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="font-medium text-slate-900">{{ $letter->nama_pengembang }}</span>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="text-sm text-slate-700">{{ $letter->nama_proyek }}</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="text-sm text-slate-700">{{ $letter->nomor_surat_masuk }}</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <div class="flex justify-center gap-2 flex-wrap">
                                        <!-- View Buttons -->
                                        <a href="{{ route('water-availability-letters.previewFormat', $letter) }}?kop=1&paper=A4" target="_blank" class="inline-flex items-center px-3 py-2 text-xs font-medium text-blue-600 hover:text-blue-900 hover:bg-blue-50 rounded transition" title="Preview surat">
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-16">
                                    <div class="text-center">
                                        ...
                                        <p class="text-slate-700 font-medium mb-2">Belum ada surat ketersediaan air</p>
                                        <p class="text-slate-600 text-sm mb-4">Mulai dengan membuat surat pertama Anda</p>
                                        <a href="{{ route('water-availability-letters.create') }}" class="inline-flex items-center px-4 py-2 bg-cyan-600 text-white font-semibold hover:bg-cyan-700 transition">
                                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                                            Buat Surat Baru
                                        </a>
                                    </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            if (!$.fn.DataTable.isDataTable('#waterAvailabilityTable')) {
                const table = $('#waterAvailabilityTable').DataTable({
                language: {
                ...
                $('#waterAvailabilitySearch').on('keyup', function() {
