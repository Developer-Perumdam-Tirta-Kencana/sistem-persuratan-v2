...
<x-slot name="header">
    <h2 class="font-semibold text-xl text-gray-800 leading-tight">
        {{ __('Edit Surat Kuasa Pelimpahan') }}
    </h2>
</x-slot>
    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                <form action="{{ route('delegation-letters.update', $delegationLetter) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        ...
                        <select name="pemberi_kuasa_1_id" id="pemberi_kuasa_1_id" 
                            ...
                            <option value="{{ $user->id }}" {{ old('pemberi_kuasa_1_id', $delegationLetter->pemberi_kuasa_1_id) == $user->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="pemberi_kuasa_2_id" id="pemberi_kuasa_2_id" 
                            ...
                            <option value="">Pilih User</option>
                            @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ old('pemberi_kuasa_2_id', $delegationLetter->pemberi_kuasa_2_id) == $user->id ? 'selected' : '' }}>
                                {{ $user->name }} ({{ $user->email }})
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="penerima_kuasa_id" id="penerima_kuasa_id" 
                            ...
                            <option value="">Pilih User</option>
                            @foreach($users as $user)
                            <option value="{{ $user->id }}" {{ old('penerima_kuasa_id', $delegationLetter->penerima_kuasa_id) == $user->id ? 'selected' : '' }}>
                                {{ $user->name }} ({{ $user->email }})
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="tujuan_transaksi" id="tujuan_transaksi" rows="4" 
                            class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('tujuan_transaksi') border-red-500 @enderror" required>{{ old('tujuan_transaksi', $delegationLetter->tujuan_transaksi) }}</textarea>
